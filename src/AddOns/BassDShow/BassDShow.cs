...
namespace ManagedBass.DShow
{
	...
	public static class BassDShow
	{
		...
		{
		    BassDShowColorRange colorRange;
		    if (!ChannelColorRange(Handle, Id, out colorRange))
		...
		{
			BassDShowStreams streams;
		    if (!ChannelGetStream(Handle, Index, out streams))
		        throw new Exception(LastError.ToString());
		...
		{
		    BassDShowPluginInfo info;
		    if (PluginGetInfo(Plugin, out info))
		        throw new Exception(LastError.ToString());
		...
		[DllImport("xVideo.dll", CharSet=CharSet.Auto, EntryPoint="xVideo_VCamStreamChannel", ExactSpelling=false)]
		public static extern bool BASS_DSHOW_VCamStreamChannel(int Handle, bool Stream);
	}
}